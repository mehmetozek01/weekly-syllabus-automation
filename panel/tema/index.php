<!-- Header kısmını import etmemiz için gerekli olan kod parçacığı -->

<?php

include 'includes/header.php';
?>

<!-- / Navbar -->

<!-- Content wrapper -->
<div class="content-wrapper">
  <!-- Content -->

  <div class="container-xxl flex-grow-1 container-p-y">

  
    <!-- Ana sayfada gösterilen Toplam kullanıcı - Programlar - Öğretmenler - Öğrenciler gösteriliyor -->

      <div class="row">
        <div class="col-xl-2 col-lg-2 col-md-4 col-sm-4 col-6 mb-4">
          <div class="card">
            <!-- Toplam Kullanıcı -->
            <div class="card-body text-center">
              <div class="avatar avatar-md mx-auto mb-3">
                <span class="avatar-initial rounded-circle bg-label-info"><i class="bx bx-group fs-3"></i></span>
              </div>
              <span class="d-block mb-1 text-nowrap">Toplam Kullanıcı</span>
              <h2 class="mb-0">12</h2>
            </div>
          </div>
        </div>
        <div class="col-xl-2 col-lg-2 col-md-4 col-sm-4 col-6 mb-4">
          <div class="card">
            <div class="card-body text-center">
              <!-- Programlar -->
              <div class="avatar avatar-md mx-auto mb-3">
                <span class="avatar-initial rounded-circle bg-label-warning"><i class="bx bx-dock-top fs-3"></i></span>
              </div>
              <span class="d-block mb-1 text-nowrap">Programlar</span>
              <h2 class="mb-0">17</h2>
            </div>
          </div>
        </div>
        <div class="col-xl-2 col-lg-2 col-md-4 col-sm-4 col-6 mb-4">
          <div class="card">
            <div class="card-body text-center">
              <!-- Öğretmenler -->
              <div class="avatar avatar-md mx-auto mb-3">
                <span class="avatar-initial rounded-circle bg-label-danger"><i class="bx bx-book-bookmark fs-3"></i></span>
              </div>
              <span class="d-block mb-1 text-nowrap">Öğretmenler</span>
              <h2 class="mb-0">29</h2>
            </div>
          </div>
        </div>
        <div class="col-xl-2 col-lg-2 col-md-4 col-sm-4 col-6 mb-4">
          <div class="card">
            <div class="card-body text-center">
              <!-- Öğrenciler -->
              <div class="avatar avatar-md mx-auto mb-3">
                <span class="avatar-initial rounded-circle bg-label-primary"><i class="bx bx-user fs-3"></i></span>
              </div>
              <span class="d-block mb-1 text-nowrap">Öğrenciler</span>
              <h2 class="mb-0">72</h2>
            </div>
          </div>
        </div>
       
      </div>

  

  </div>

<!-- Footer kısmını import etmemiz için gerekli olan kod parçacığı -->
<?php

  include 'includes/footer.php';
  ?>
